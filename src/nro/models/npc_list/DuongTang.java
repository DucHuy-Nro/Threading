...
import nro.models.services.MapService;
...
public class DuongTang extends Npc {
    ...
    public void openBaseMenu(Player player) {
        ...
        else if (this.mapId == 122 || this.mapId == 123 || this.mapId == 124) {
            createOtherMenu(player, ConstNpc.BASE_MENU,
                "A mi phò phò!\n"
    ...
    public void confirmMenu(Player player, int select) {
        ...
        if (this.mapId == 0 && player.iDMark.isBaseMenu()) {
            handleLangAruMenu(player, select);
        }
        // Xử lý tại Ngũ Hành Sơn
        else if ((this.mapId == 122 || this.mapId == 123 || this.mapId == 124) 
                 && player.iDMark.isBaseMenu()) {
            handleNguHanhSonMenu(player, select);
        }
    }

    // ============ XỬ LÝ MENU LÀNG ARU ============
    private void handleLangAruMenu(Player player, int select) {
        switch (select) {
    ...
    // ============ XỬ LÝ MENU NGŨ HÀNH SƠN ============
    private void handleNguHanhSonMenu(Player player, int select) {
        switch (select) {
            case 0: // Đổi đào chín
                Service.getInstance().sendThongBao(player, 
